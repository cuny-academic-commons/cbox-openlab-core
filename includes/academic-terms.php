...
/**
 * Enforce unique sort order for academic terms.
 *
 * Fired whenever a term is saved, created, or updated.
 *
 * This prevents race conditions and other confusing behavior when terms share menu_order.
 */
function cboxol_enforce_unique_order_for_academic_terms( $post_id ) {
	$post = get_post( $post_id );
	if ( ! $post || 'cboxol_acad_term' !== $post->post_type ) {
		return;
	}

	$terms          = cboxol_get_academic_terms();
	$existing_order = array();
	$correct_order  = array();
	$i              = 0;
	foreach ( $terms as $term ) {
		$term_id = $term->get_wp_post_id();

		$correct_order[ $term_id ] = $i;
		$i++;

		$existing_order[ $term_id ] = $term->get_order();
	}

	if ( $correct_order === $existing_order ) {
		return;
	}
	...
	foreach ( $term as $term ) {
		...
		$correct_term_order = $correct_order[ $term_id ];

		if ( $correct_term_order === $term->get_order() ) {
			continue;
		}

		$term->set_order( $correct_term_order );
		$term->save();
	}
}
add_action( 'save_post', 'cboxol_enforce_unique_order_for_academic_terms' );
add_action( 'before_delete_post', 'cboxol_enforce_unique_order_for_academic_terms' );
