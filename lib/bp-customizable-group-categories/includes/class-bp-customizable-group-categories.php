...
class Bp_Customizable_Group_Categories {
	...
	public function admin_warning() {
		...
		if ( ! $this->version_check() ) {
			...
			$warnings[] = sprintf( __( 'BP Customizable Group Categories requires at least version %1$s of BuddyPress and version %2$s of WordPress.', 'commons-in-a-box' ), self::$required_version['bp'], self::$required_version['wp'] );
		}

		if ( ! bp_core_do_network_admin() && ! $this->root_blog_check() ) {
			$warnings[] = __( 'BP Customizable Group Categories requires to be activated on the blog where BuddyPress is activated.', 'commons-in-a-box' );
		}
	...
	public function register_taxonomy() {
		...
		$labels = array(
			'name'          => _x( 'Group Categories', 'taxonomy general name', 'commons-in-a-box' ),
			'singular_name' => _x( 'Group Tag', 'taxonomy singular name', 'commons-in-a-box' ),
		);
