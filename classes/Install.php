...
class Install {
		...
		$types_data = array(
			'course'    => array(
				...
				'labels'                           => array(
					...
					'group_contact'                        => __( 'Course Contact', 'commons-in-a-box' ),
					'group_contact_help_text'              => __( 'By default, you are the Course Contact. You may add or remove Course Contacts once your portfolio has more members.', 'commons-in-a-box' ),
	...
	protected function install_default_widgets() {
		...
		if ( ! CBox_Widget_Setter::is_sidebar_populated( 'footer' ) ) {
			...
			$welcome_text = sprintf( __( 'Modify the text of this and other widgets using the <a href="%s">Customizer</a>.', 'commons-in-a-box' ), get_admin_url( cbox_get_main_site_id(), 'customize.php?autofocus[section]=sidebar-widgets-footer' ) );
...
<p>' . $left_text . '</p>
		...
		$middle_text    = sprintf( __( 'You can customize the contents of this footer to meet the needs of your community: see the <a href="%s">Brand Settings documentation</a> for details.', 'commons-in-a-box' ), esc_url( admin_url( 'admin.php?page=cbox-ol-brand-settings' ) ) );
